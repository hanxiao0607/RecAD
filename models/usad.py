...
def testing(model, test_loader, alpha=.5, beta=.5, device='cpu'):
    results = []
...
def testing_sample(model, sample, alpha=.5, beta=.5):
    w1 = model.decoder1(model.encoder(sample))
    w2 = model.decoder2(model.encoder(w1))
    result = alpha * torch.mean((sample - w1) ** 2, axis=1) + beta * torch.mean((sample - w2) ** 2, axis=1)
    return result