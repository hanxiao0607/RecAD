...
class RecAD(object):
    def __init__(self, options):
        ...
        self.recourse_model_look_forward = options['recourse_look_forward']
        ...
        if self.get_baseline_GVAR:
            print('Get results for the baseline model GVAR:')
            self._get_baseline_GVAR()
        if self.get_baseline_VAR:
            print('Get results for the baseline model VAR:')
            self._get_baseline_VAR()
        if self.get_baseline_linear:
            print('Get results for the baseline model linear:')
            self._get_baseline_other(model_name='linear', training=1, max_epoch=50)
        if self.get_baseline_fc:
            print('Get results for the baseline model fc:')
            self._get_baseline_other(model_name='fc', training=1, max_epoch=50)
        if self.get_baseline_lstm:
    ...
    def _get_recourse_model(self, cost_f=False):
        ...
        # ).to(self.device)
        self.recourse_model = RecAD_model.RecAD(input_size=self.p, hidden_size=self.recourse_model_hidden_dim, num_layers=1).to(self.device)
        ...
        if self.recourse_model_training:
            ...
            for _ in tqdm(range(self.recourse_model_max_epoch), desc="RecAD training epoch:"):
                ...
                for i in range(len(self.rec_train_x)):
                    ...
                    for j in range(self.ad_model_K, len(sample_cf)):
                        ...
                        if w_recon > self.mse:
                            ...
                            theta = self.recourse_model(window_cf[:, :-1, :], delta_pred)

                            if self.recourse_model_look_forward >= 1:
                                rec_loss_total = 0.0
                                for k in range(1, self.recourse_model_look_forward+1):
                                    # get next time step loss
                                    if j + k > len(sample_org):
                                        break
                                    window_org_next = torch.tensor(
                                        sample_org[np.newaxis, j - self.ad_model_K + k:j + k, :]).float().to(self.device)
                                    window_cf_next = torch.tensor(
                                        sample_cf[np.newaxis, j - self.ad_model_K + k:j + k, :]).float().to(self.device)
                                    window_cf_next = window_cf_next + F.pad(theta, (0, 0, self.ad_model_K - k - 1, k))
                                    ...
                                    rec_loss_total = rec_loss_total + rec_loss_next
                            else:
                                rec_loss_total = 0.0
                            ...
                            if self.recourse_model_look_forward >= 1:
                                loss = self.recourse_model_alpha * rec_loss + self.recourse_model_beta * l2_loss + self.recourse_model_gamma/self.recourse_model_look_forward * rec_loss_total
                            else:
                            ...
                            epoch_rec_lf_loss += rec_loss_total
                                ...
                                if self.recourse_model_look_forward >= 1:
                                    ...
                                    for k in range(1, self.recourse_model_look_forward+1):
                                        ...
                                        if j + k > len(sample_org):
                                            break
                                        window_org_next = torch.tensor(
                                            sample_org[np.newaxis, j - self.ad_model_K + k:j + k, :]).float().to(
                                            self.device)
                                        window_cf_next = torch.tensor(
                                            sample_cf[np.newaxis, j - self.ad_model_K + k:j + k, :]).float().to(
                                            self.device)
                                        y_org_next_pred = self.senn_1(window_org_next[:, -self.K - 1:-1, :])[0]
                                        delta_pred_next = window_org_next[:, -1, :] - y_org_next_pred.data
                                        y_cf_next_pred = self.senn_1(window_cf_next[:, -self.K - 1:-1, :])[
                                                             0] + delta_pred_next
                                        window_cf_next[:, -1, :] = y_cf_next_pred.data
                                        window_rec_next_flattened = window_cf_next.reshape(1, -1)
                                        rec_loss_next = usad.testing_sample(self.ad_model, window_rec_next_flattened,
                                                                            alpha=self.ad_model_alpha,
                                                                            beta=self.ad_model_beta)
                                        rec_loss_total = rec_loss_total + rec_loss_next
                                else:
                ...
                if eval_loss/eval_count < best_eval_loss:
                    best_eval_loss = eval_loss/eval_count
                    torch.save(self.recourse_model.state_dict(), f'./saved_models/RC_{self.save_parm}.pt')

                print(f'Train reconstruction loss: {epoch_rec_loss/count}, look forward rec loss: {float(epoch_rec_lf_loss)/count}, l2 loss: {epoch_l2_loss/count}, logit loss: {epoch_logit_loss}, recourse wind_count: {count-1}')
    ...
    def _get_baseline_GVAR(self):
        ...
        count = 0.00001
        ad_sample_count = 0
        for i in tqdm(range(self.rec_test_x.shape[0]), desc='Testing sample for baseline GVAR:'):
            sample_cf = self.rec_test_x[i].copy()
            sample_gt = sample_cf.copy()
            if self.dataset_name != 'MSDS':
                sample_eps = self.test_eps_norm[i]
            early_stop = 0
            sample_count = 0
            with torch.no_grad():
                for j in range(self.ad_model_K, len(sample_cf)):
                    ...
                    if self.dataset_name != 'MSDS':
                        ...
                        window_gt = torch.tensor(window_gt).float().to(self.device)
                        window_gt_flattened = window_gt.reshape(1, -1)
                        w_recon = usad.testing_sample(self.ad_model, window_gt_flattened, alpha=self.ad_model_alpha,
                                                      beta=self.ad_model_beta)
                    ...
                    if w_recon > self.mse:
                        ...
                        if early_stop == 0:
                            early_stop = 1
                        y_cf_next_pred = self.senn_1(window_gt[:, -self.K - 1:-1, :])[0]
                        theta = window_gt[:, -1, :] - y_cf_next_pred
                        sample_gt[j - 1] = y_cf_next_pred.detach().cpu().numpy()[0]
                        window_rec = window_gt.clone()
                        window_rec[:, -1, :] = y_cf_next_pred
                        window_rec_flattened = window_rec.reshape(1, -1)
                        rec_loss = usad.testing_sample(self.ad_model, window_rec_flattened, alpha=self.ad_model_alpha,
                                                       beta=self.ad_model_beta)
                        logit_loss = 1 if rec_loss > self.mse else 0
    ...
    def _get_baseline_VAR(self):
        self.ad_model.eval()
        self.senn_1.eval()
        if self.dataset_name == 'MSDS':
            VAR_model = VAR(self.train_x_norm)
        else:
            VAR_model = VAR(self.train_x_norm[0])
        ...
        count = 0.00001
        ad_sample_count = 0
        for i in tqdm(range(self.rec_test_x.shape[0]), desc='Testing sample for baseline VAR:'):
            sample_cf = self.rec_test_x[i].copy()
            sample_gt = sample_cf.copy()
            if self.dataset_name != 'MSDS':
                sample_eps = self.test_eps_norm[i]
            ...
            with torch.no_grad():
                for j in range(self.ad_model_K, len(sample_cf)):
                    ...
                    if w_recon > self.mse:
                        ...
                        if early_stop == 0:
                            early_stop = 1
                        y_cf_next_pred = results.forecast(window_gt[:, :-1, :][0].cpu(), 1)
                        theta = window_gt[:, -1, :] - torch.tensor(y_cf_next_pred).to(self.device)
                        sample_gt[j - 1] = y_cf_next_pred[0]
                        window_rec = window_gt.clone()
                        window_rec[:, -1, :] = torch.tensor(y_cf_next_pred)
                        window_rec_flattened = window_rec.reshape(1, -1)
                        rec_loss = usad.testing_sample(self.ad_model, window_rec_flattened, alpha=self.ad_model_alpha,
                                                       beta=self.ad_model_beta)
                        logit_loss = 1 if rec_loss > self.mse else 0
    ...
    def _get_baseline_other(self, model_name='linear', training=1, max_epoch=50):
        self.ad_model.eval()
        self.senn_1.eval()
        if model_name == 'linear':
            baseline_model = inet.INet(num_vars=self.p, k=(self.ad_model_K-1), hidden_layer_size=self.recourse_model_hidden_dim,
                  num_hidden_layers=self.num_hidden_layers, device=self.device).to(self.device)
        elif model_name == 'fc':
            baseline_model = nn.Sequential(
                nn.Dropout(p=0.1),
                nn.Linear((self.ad_model_K-1) * self.p, self.recourse_model_hidden_dim),
                nn.ReLU(),
                nn.Linear(self.recourse_model_hidden_dim, self.recourse_model_hidden_dim),
                nn.ReLU(),
                nn.Linear(self.recourse_model_hidden_dim, self.recourse_model_hidden_dim),
            ...
            baseline_model = lstm.MTLSTM(input_size=self.p, hidden_size=self.recourse_model_hidden_dim).to(self.device)
        else:
            NotImplementedError
        baseline_model_optim = optim.Adam(baseline_model.parameters())
        baseline_model_loss = nn.MSELoss()
        w_size = (self.ad_model_K-1) * self.p

        print(f'Training baseline {model_name} model.')
        windows, ys = utils.sliding_window(self.train_x_norm, get_ys=1, y=None,
                                          size=self.ad_model_K-1, downsampling=self.ad_model_downsampling)
        split_ind = int(np.floor(.8 * windows.shape[0]))
        if model_name == 'fc':
            windows_normal_train = windows[:split_ind].reshape(split_ind, -1)
            windows_normal_val = windows[split_ind:].reshape(len(windows) - split_ind, -1)
        else:
            windows_normal_train = windows[:split_ind]
            windows_normal_val = windows[split_ind:]
        ys_train = ys[:split_ind]
        ys_val = ys[split_ind:]

        train_loader = torch.utils.data.DataLoader(utils.SlidingWind(windows_normal_train, ys_train),
                                                   batch_size=self.batch_size, shuffle=False, num_workers=0)

        val_loader = torch.utils.data.DataLoader(utils.SlidingWind(windows_normal_val, ys_val),
                                                   batch_size=self.batch_size, shuffle=False, num_workers=0)
        if training:
            best_val_loss = float('inf')
            for _ in tqdm(range(max_epoch), desc=f'Training for baseline {model_name}:'):
                epoch_training_loss = 0.0
                epoch_val_loss = 0.0
                baseline_model.train()
                for batch in train_loader:
                    windows = batch[0].to(self.device)
                    ys = batch[1].to(self.device)
                    baseline_model_optim.zero_grad()
                    pred = baseline_model(windows)
                    loss = baseline_model_loss(ys, pred)
                    loss.backward()
                    baseline_model_optim.step()
                    epoch_training_loss += loss.item()/len(batch)
                baseline_model.eval()
                with torch.no_grad():
                    ...
                    if epoch_val_loss < best_val_loss:
                        ...
                        torch.save(baseline_model.state_dict(), f'./saved_models/baseline_{model_name}_{self.save_parm}.pt')
                print(f'Training loss: {epoch_training_loss/len(train_loader)}, validation loss: {epoch_val_loss/len(val_loader)}')
            print('Finish training baseline model.')

        print('Loading the best baseline model')
        baseline_model.load_state_dict(torch.load(f'./saved_models/baseline_{model_name}_{self.save_parm}.pt', map_location=self.device))
        baseline_model.eval()
        ...
        count = 0.00001
        ad_sample_count = 0
        for i in tqdm(range(self.rec_test_x.shape[0]), desc=f'Testing sample for baseline {model_name}:'):
            sample_cf = self.rec_test_x[i].copy()
            sample_gt = sample_cf.copy()
            if self.dataset_name != 'MSDS':
                sample_eps = self.test_eps_norm[i]
                ...
                for j in range(self.ad_model_K, len(sample_cf)):
                    ...
                    if w_recon > self.mse:
                        ...
                        if early_stop == 0:
                            early_stop = 1
                        if model_name == 'fc':
                            y_cf_next_pred = baseline_model(window_gt[:, :-1, :].reshape(1, -1))
                        else:
                            y_cf_next_pred = baseline_model(window_gt[:, :-1, :])
                        theta = window_gt[:, -1, :] - y_cf_next_pred
                        sample_gt[j - 1] = y_cf_next_pred.cpu().numpy()
                        window_rec = window_gt.clone()
                        window_rec[:, -1, :] = y_cf_next_pred
                        window_rec_flattened = window_rec.reshape(1, -1)
                        rec_loss = usad.testing_sample(self.ad_model, window_rec_flattened,
